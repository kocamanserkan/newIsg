...
namespace serkanISG
{
    public partial class ISTANIMLARI : System.Web.UI.Page
    {
        ...
        protected void edit_Kaydet_Click(object sender, EventArgs e)
        {
            ...
            //string c = Server.HtmlDecode(txtIsAdi_edit.Text);
            //ISTANIM editIS = new ISTANIM();
            //int newID = Convert.ToInt32(lblISid_edit.Text);
            //byte[] bytes = Encoding.Default.GetBytes(txtIsAdi_edit.Text);
            //txtIsAdi_edit.Text = Encoding.UTF8.GetString(bytes);

            //editIS = db.ISTANIM.FirstOrDefault(i => i.ISID == newID);

            //editIS.ISADI = HttpUtility.HtmlDecode(txtIsAdi_edit.Text);
